...
def create_label_and_entry(master, text, row, entryDoctorId=None, is_image=False):
    ...
    if is_image:
        ...
        lblImageStatus.grid(row=row, column=1, padx=(10, 5), pady=5, sticky="w")
...
def btnClinic_Click(clinic_id):
    ...
    try:
        ...
        if clinic_doc.exists:
            ...
            if clinic.get_image() and clinic.get_image() != 'N/A':  # Ensure image path is valid
                image_path = clinic.get_image()  # Assuming this returns the image path
                blob = bucket.blob(image_path)
                if blob.exists():
                    try:
                        # Download image data and convert it
                        img_data = blob.download_as_bytes()
                        img = Image.open(BytesIO(img_data))
                        img = img.resize((200, 200), Image.LANCZOS)

                        # Add rounded corners to the image
                        img = addRoundedCorners(img, 10)

                        # Convert PIL Image to PhotoImage
                        clinic_image = ImageTk.PhotoImage(img)
...
def btnEditClinic_click(clinic):
    ...
    try:
        ...
        if clinic_doc.exists:
            ...
            btnUploadImage = CTkButton(master=frameClinic, text="Select Image",
...
def btnSaveClinic_Click(clinic_id, entryName, lblImageStatus, entryAddress, entryEmail, entryPhone, entrySpecialty,
    ...
    if lblImageStatus.cget("text") == "Image has been selected":
        imageFile = upload_clinic_image(selected_image_path, clinic_id, lblImageStatus)
    else:
        imageFile = current_image_path
    ...
    try:
        ...
        if clinic_doc.exists:
            ...
            updated_data = {
                ...
                'Image': imageFile if imageFile else clinic_data.get('Image')  # Ensure image path is updated
            }
...
def uploadClinic_image(label_status, clinic, lblImageStatus):
    ...
    if filename:
        ...
        if new_image_url:
            clinic.set_image(new_image_url)  # Update doctor object with new image URL
    else:
        label_status.configure(text="No file selected for upload.", text_color="red")
