...
class Appointment:
    def __init__(self, appointment_id, appointment_type, clinic_id, date_time, date, time, clinic_name, doctor_id,
                 doctor_name,
                 medical_concern, medical_doc, patient_address, patient_id, patient_name, prescription, status):
...
def get_appointment_list(filter_type, appointments_ref, frameAppointmentList, search_term=None):
    try:
        ...
        lblNo = CTkLabel(master=frameAppointmentList, text="No.", font=("Inter", 20), text_color="black")
        ...
        for appointment_info in appointment_data_list:
            ...
            if appointment_doc.exists:
                ...
                lblIndex = CTkLabel(master=frameAppointmentItem, text=f"{index}.", font=("Inter", 20),
                                    text_color="black")
        ...
        img_path = "C:/Users/michi/PycharmProjects/DCS2103_Aug2023/SE Project/Images/Beautiful hospital.jpg"
...
def create_label_and_entry(master, text, row, entryDoctorId=None, is_image=False):
    ...
    if is_image:
        ...
        lblImageStatus.grid(row=row, column=1, padx=20, pady=10)
        ...
        btnUploadImage.grid(row=row, column=2, padx=10, pady=10)
...
def new_doctor(clinic_Id):
    ...
    btnBack = CTkButton(
        ...
        image=CTkImage(imgBtnBack, size=(30, 30)),
        width=50, height=50, corner_radius=0,
        command=lambda: btnDoctor_Click(clinic_Id)  # Removed nested lambda
    )
...
def uploadDoctor_image(label_status, doctor, lblImageStatus):
    ...
    if filename:
        ...
        label_status.configure(text="Image has been selected", fg_color="green")
        # Update doctor's image and display
        new_image_url = upload_doctor_image(filename, doctor.get_doctor_id(), label_status)
        if new_image_url:
            doctor.set_image(new_image_url)
            update_doctor_image_display(lblImageStatus)
    else:
        label_status.configure(text="No file selected for upload.", fg_color="red")


def update_doctor_image_display(image_url):
    # Fetch the image from Firebase Storage using the image_url
    bucket = storage.bucket()
    ...
    if blob is not None:
        try:
            ...
            # Update the label with the new image and preserve the text
            for widget in frameEditDoctor.winfo_children():
                if isinstance(widget, CTkLabel) and widget.cget("text") == "Image:":
                    widget.configure(image=photo_image, compound='top')
                    widget.image = photo_image
                    break
        except Exception as e:
            print(f"Error decoding image for doctor: {e}")
    else:
        print(f"Blob not found for path: {image_url}")
...
def edit_doctor(doctor, doctor_data, doctor_id, clinic_id):
    ...
    btnBack = CTkButton(
        ...
        image=CTkImage(imgBtnBack, size=(30, 30)),
        width=50, height=50, corner_radius=0,
        command=lambda: doctor_details_click(doctor_data, clinic_id)
    )
    btnBack.grid(row=row_index, column=0, padx=20, pady=(10, 5), sticky="w")
    ...
    btnUploadImage = CTkButton(master=frameEditDoctor, text="Select Image",
                               command=lambda: uploadDoctor_image(lblImageStatus, doctor, lblImageStatus))
    btnUploadImage.grid(row=row_index, column=1, padx=(60, 0), pady=10, sticky='e')
...
def btnSaveDoctor_Click(doctor, doctor_id, entryName, lblImageStatus, entryEmail, entryPhone, entryQualification,
    ...
    if lblImageStatus.cget("text") == "An image has been selected.":
        imageDoctorFile = upload_doctor_image(selected_image_path, doctor_id, lblImageStatus)
    else:
        imageDoctorFile = doctor.get_image()
        ...
        if clinic_doc.exists:
            ...
            for clinic_info in clinic_data_list:
                ...
                if blob.exists():
                    ...
                    ctk_img_clinic_list = CTkImage(pil_img_clinic_list, size=(400, 250))
                else:
            ...
            clinic_info = (
                ...
                f"Phone No.: {clinic.get_phone_no()}\n"
            ...
            btn_edit.grid(row=row_index, column=2, pady=20,
                          sticky="new")
...
global imageClinicFile


def upload_clinic_image(file_path, clinic_id, label_status):
    global imageClinicFile

    if file_path:
        file_extension = file_path.split('.')[-1]
        file_name = f"Clinics/{clinic_id}.{file_extension}"  # Adjust the path as per your storage structure
        ...
        try:
            blob.upload_from_filename(file_path)
            blob.make_public()
            print("Clinic image uploaded successfully. URL:", blob.public_url)

            # Update label status to indicate successful upload
            label_status.configure(text="Image uploaded successfully.", fg_color="green")
            imageFile = file_name  # Store the uploaded image path for later use
        except Exception as e:
            print("Error uploading clinic image:", e)
            label_status.configure(text="Error uploading image.", fg_color="red")
    else:
        print("No file selected for upload.")
        label_status.configure(text="No file selected for upload.", fg_color="red")
...
def add_label_entry(row, label_text, entry_text):
    label = CTkLabel(master=frameClinic, text=label_text, font=("Inter", 20), justify="left", anchor="w")
    label.grid(row=row, column=0, sticky="w", padx=20, pady=10)

    entry = CTkEntry(master=frameClinic, font=("Inter", 15), width=300, border_width=0, corner_radius=50)
...
def uploadClinic_image(label_status, clinic, lblImageStatus):
    ...
    if filename:
        selected_image_path = filename
        label_status.configure(text="Image has been selected", fg_color="green")
        # Update clinic image and display
        new_image_url = upload_clinic_image(filename, clinic.get_clinic_Id(), label_status)
        if new_image_url:
            clinic.set_image(new_image_url)
            update_doctor_image_display(lblImageStatus)
    else:
        label_status.configure(text="No file selected for upload.", fg_color="red")
...
def btnEditClinic_click(clinic):
    ...
    try:
        ...
        if clinic_doc.exists:
            ...
            current_image_path = clinic.get_image()
            lblImageStatus = CTkLabel(master=frameClinic,
                                      text="Image has been selected" if current_image_path else "No file selected",
                                      font=("Inter", 20))
            lblImageStatus.grid(row=row_index, column=1, padx=(0, 20), pady=10)
            ...
            btnUploadImage.grid(row=row_index, column=2, padx=10, pady=10)
            row_index += 1
            ...
            row_index += 1
            entryPhone = add_label_entry(row_index, "Phone No.:", clinic.get_phone_no())
            row_index += 1
            ...
            for i, day in enumerate(workdays_abbrev):
                ...
                lbl = CTkLabel(master=frameClinic, text=day, font=("Inter", 15), justify="left", anchor="w")
                lbl.grid(row=row_index, column=0, padx=5, pady=5, sticky="w")

                chk_day = Checkbutton(master=frameClinic, variable=var, width=5)
                chk_day.grid(row=row_index, column=1, padx=5, pady=5, sticky="nsew")
                ...
                start_work_hour.grid(row=row_index, column=2, padx=5, pady=5, sticky="w")

                end_work_hour = CTkOptionMenu(
                    master=frameClinic, values=time_options, state='normal' if var.get() == 1 else 'disabled'
                )
                end_work_hour.set(end_hours[i] if var.get() == 1 else "time")
                end_work_hour.grid(row=row_index, column=3, padx=5, pady=5, sticky="w")

                start_break_hour = CTkOptionMenu(
                    master=frameClinic, values=time_options, state='normal' if var.get() == 1 else 'disabled'
                )
                start_break_hour.set(start_breaks[i] if var.get() == 1 else "time")
                start_break_hour.grid(row=row_index, column=4, padx=5, pady=5, sticky="w")

                end_break_hour = CTkOptionMenu(
                    master=frameClinic, values=time_options, state='normal' if var.get() == 1 else 'disabled'
                )
                end_break_hour.set(end_breaks[i] if var.get() == 1 else "time")
                end_break_hour.grid(row=row_index, column=5, padx=5, pady=5, sticky="w")

                work_hours_entries.append((start_work_hour, end_work_hour))
                break_hours_entries.append((start_break_hour, end_break_hour))

                chk_day.configure(
                    command=lambda v=var, s=start_work_hour, e=end_work_hour, sb=start_break_hour,
                                   eb=end_break_hour: toggle_entries(v, s, e, sb, eb))
            ...
            btnSave.grid(row=row_index, column=5, pady=20, padx=(0, 20), sticky="e")  # Adjusted column to maximum right

        else:
            print(f"No document found for clinic_id: {clinic.get_clinic_Id()}")
...
def btnSaveClinic_Click(clinic_id, entryName, lblImageStatus, entryAddress, entryEmail, entryPhone, entrySpecialty,
                        workdays_vars, work_hours_entries, break_hours_entries, entryDescription, current_image_path):
    global imageClinicFile

    if lblImageStatus.cget("text") == "An image has been selected":

        imageClinicFile = upload_clinic_image(selected_image_path, clinic_id, lblImageStatus)
    else:

        imageClinicFile = clinic.get_image()
    ...
    try:
        ...
        if clinic_doc.exists:
            ...
            # Prepare updated data for Firestore update
            updated_data = {
                'Name': entryName.get(),
                'Address': entryAddress.get(),
                'Email': entryEmail.get(),
                ...
                'Image': imageClinicFile
            }

            print(imageClinicFile)

            # Prepare work hours and break hours as comma-separated strings
            ...
            for i, day in enumerate(workdays):
                if workdays_vars[day].get() == 1:
                    start_hours.append(str(work_hours_entries[i][0].get()))
                    end_hours.append(str(work_hours_entries[i][1].get()))
                    start_break.append(str(break_hours_entries[i][0].get()))
                    end_break.append(str(break_hours_entries[i][1].get()))

            updated_data['WorkDays'] = ', '.join(day for day in workdays if workdays_vars[day].get() == 1)
            updated_data['StartHours'] = ', '.join(start_hours)
            updated_data['EndHours'] = ', '.join(end_hours)
            updated_data['StartBreak'] = ', '.join(start_break)
            updated_data['EndBreak'] = ', '.join(end_break)

            # Update the clinic document in the database only if there are changes
...
navBtnDoctor = CTkButton(master=navBtnFrame, text="Doctors", text_color="Black",
...
# Create the button with the image and label
navBtnClinic = CTkButton(master=navBtnFrame, text="Clinic Settings", text_color="Black",
